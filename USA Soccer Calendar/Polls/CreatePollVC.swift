...
class CreatePollVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //answer1TF.smartInsertDeleteType = UITextSmartInsertDeleteType.no
    ...
    @objc func confirm() {
        ...
        if answer4TF.text != "" && answer3TF.text == "" {
            AlertController.showAlert(self, title: "Error", message: "must provide 3rd answer option if also providng 4th answer option")
            return
        }
        ...
        if let user = Auth.auth().currentUser {
            ...
            Firestore.firestore().collection("Polls").document().setData(questionData)
            dismissPopUp()
